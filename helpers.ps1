...
function infusionGetToken($key, $secret) {
    # ==============
    # auth as a client
    # ==============
    if ($null -eq $env:InfusionAPI) {
        $env:InfusionAPI = "https://api.infusionsoft.com"
    }

    [HttpClientHandler]$handler = New-Object 'HttpClientHandler'

    "==> authenticating against $($env:InfusionAPI)..." | Write-Host -BackgroundColor Green
    $client = [TokenClient]::new("$($env:InfusionAPI)/token", $clientId, $clientSecret, $handler, [AuthenticationStyle]::BasicAuthentication)
    #"client.Address: $($client.Address)" | Write-Host
    # | Write-Host -BackgroundColor Green
